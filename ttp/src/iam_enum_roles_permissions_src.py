...
def main(args, awsattack_main, data=None):
    ...
    if not any([args.all_roles, args.role_name]):
        ...
        if re.match(r'arn:aws:iam::\d{12}:user/', identity['Arn']) is not None:
            ...
            try:
                ...
                print('Failed to discover the current users username, enter it now or Ctrl+C to exit the module: ')
                return False
                if username:
                    active_aws_key.update(
                        awsattack_main.database,
    ...
    client = awsattack_main.get_boto3_client('iam')
    if any([args.all_roles, args.role_name]):
