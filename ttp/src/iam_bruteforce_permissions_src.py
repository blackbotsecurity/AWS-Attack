...
def invalid_param(valid_type):
    ...
    types = {
        'datetime.datetime': datetime.datetime(2015, 1, 1),
        'list': ['test'],
        'int': 1,
        'dict': {},
        'bool': True
...
def generate_preload_actions():
    ...
    """
    technique_dir = os.path.dirname(__file__)
    path = os.path.join(technique_dir, 'preload_actions.json')
...
def read_only_function(service, func):
    ...
    """
    technique_dir = os.path.dirname(__file__)
    path = os.path.join(technique_dir, 'ReadOnlyAccessPolicy.json')
...
def main(args, awsattack_main, data=None):
    ...
    for service in service_list:
        ...
        for region in regions:
            ...
            current_client = awsattack_main.get_boto3_client(service, region)
