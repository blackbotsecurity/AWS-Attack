...
class AWSKey(Base, ModelUpdateMixin):
    ...
    session_id = Column(Integer, ForeignKey('awsattack_session.id', ondelete='CASCADE'))
...
class awsattackSession(Base, ModelUpdateMixin):
    __tablename__ = 'awsattack_session'
    ...
    def __repr__(self):
        ...
        if self.is_active:
            return '<awsattackSession #{} ({}:{}) (ACTIVE)>'.format(self.id, self.name, key_alias)
        return '<awsattackSession #{} ({}:{})>'.format(self.id, self.name, key_alias)

    @classmethod
    def get_active_session(cls, database):
        # SQLAlchemy's query filters disallow the use of `cond is True`.
        return database.query(awsattackSession).filter(awsattackSession.is_active == True).scalar()

    def get_active_aws_key(self, database):
        """ Return the AWSKey with the same key_alias as the awsattackSession.
        ...
        # return database.query(AWSKey).filter(AWSKey.key_alias == self.key_alias).filter(AWSKey.awsattack_session_id == self.id).scalar()

    def activate(self, database):
        for other_session in database.query(awsattackSession).filter(awsattackSession.id != self.id):
