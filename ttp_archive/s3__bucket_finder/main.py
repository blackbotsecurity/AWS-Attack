...
technique_info = {
    # Name of the technique (should be the same as the filename)
    'name': 's3__bucket_finder',

    # Name and any other notes about the author
    'author': 'Dwight Hohnstein',

    # Category of the technique. Make sure the name matches an existing tactic.
    'tactic': 'RECON_UNAUTH',

    # One liner description of the technique functionality. This shows up when a user searches for ttp.
    'intent': 'Enumerates/bruteforces S3 buckets based on different parameters.',

    # Description about what the technique does and how it works
    'description': 'This technique searches across every AWS region for a variety of bucket names based on a domain name, subdomains, affixes given and more. Currently the tool will only present to you whether or not the bucket exists or if they are listable.',

    # A list of AWS services that the technique utilizes during its execution
    'services': ['S3'],

    # For prerequisite ttp, try and see if any existing ttp return the data that is required for your technique before writing that code yourself, that way, session data can stay separated and modular.
    'prerequisite_ttp': [],

    # External resources that the technique depends on. Valid options are either a GitHub URL (must end in .git) or single file URL.
...
parser = argparse.ArgumentParser(add_help=False, description=technique_info['description'])
...
parser.add_argument('--subbrute', action='store_true', default=False, help='Enable sublist3r\'s subbrute technique when querying for subdomains.')
...
def main(args, awsattack_main):
    ...
    install_dependencies = awsattack_main.install_dependencies
    ######
    ...
    # Attempt to install the required external dependencies, exit this technique if that fails
    if not install_dependencies(technique_info['external_dependencies']):
