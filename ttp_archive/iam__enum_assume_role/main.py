...
technique_info = {
    ...
    'intent': 'Enumerates existing roles in other AWS accounts to try and gain access via misconfigurations.',

    'description': 'This technique takes in an AWS account ID and tries to enumerate role names within that account. If one is discovered and it is misconfigured to allow role-assumption from a wide group, it is possible to assume that role and gain access to that AWS account through this method. NOTE: This technique is listed under the recon_enum_no_keys tactic because it is not recommended to use compromised keys to run this technique. This technique DOES require a set of AWS keys, but it will spam CloudTrail with "AssumeRole" logs, so it is suggested to use a personal account to run this. The keys you use should have the sts:AssumeRole permission on any resource ("*") to identify/assume misconfigured roles, but you will still be able to enumerate roles that exist without it.',
...
parser.add_argument('--word-list', required=False, default=None, help='File path to a different word list to use. There is a default word list with 1100+ words and it is stored in this technique\'s folder. The word list should contain words, one on each line, to use to try and guess IAM role names. Role names ARE case-sensitive.')
...
def main(args, awsattack_main):
    ...
    if args.word_list is None:
        word_list_path = './ttp/{}/default-word-list.txt'.format(technique_info['name'])
    else:
